...
function buildImportMap(): DOMConversionMap {
  ...
  const textNodeImportDOM =
    (TextNode as unknown as { importDOM?: () => DOMConversionMap | null })
      .importDOM;

  // Wrap all TextNode importers with a function that also imports
  // the custom styles implemented by the playground
  for (const [tag, fn] of Object.entries(textNodeImportDOM?.() || {})) {
...
function ControlledEditorPlugin({
  text = '',
  onChange,
  editable = true,
}: FullEditorProps): null {
  ...
  useEffect(() => {
    ...
    editor.update(() => {
      const root = $getRoot();
      root.clear();
      $convertFromMarkdownString(text, PLAYGROUND_TRANSFORMERS, root);
    });
  }, [editor, text]);
...
export default function FullEditor({
  ...
  onChange,
  editable,
}: FullEditorProps): JSX.Element {
  return (
    <div className="full-editor-root">
      <SettingsContext>
        <FlashMessageContext>
          <App text={text} onChange={onChange} editable={editable} />
        </FlashMessageContext>
      </SettingsContext>
    </div>
  );
